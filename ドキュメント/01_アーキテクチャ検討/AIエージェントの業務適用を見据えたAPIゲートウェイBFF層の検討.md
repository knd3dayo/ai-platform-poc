...
* **API Gateway**：Azure APIM（またはNginx/Kong）
  * JWT検証、レート制限、WAFなどの決定論的防御
* **BFF**：FastAPI
  * `trace_id` 発行/伝播、IDマッピング（Redis等のKVS）
  * 受付即答（`202 Accepted`）
* **フロントへの通知**：SSE（Server-Sent Events）
  * PoCではWebSocketより実装/運用が軽量
* **非同期連携基盤**：Redis Streams（推奨）
  * Webhook入力やバックエンド完了通知をEvent Busへ正規化
* **観測**：Langfuse + OpenTelemetry
  * `trace_id` をキーにBFF〜Application〜Assuranceを串刺し

> 補足：SSE/WebSocketは、現時点のコード実装に依存しない「推奨インターフェース」として記載しています。
> 実装は段階的に導入可能です（PoCではポーリングでも可）。
