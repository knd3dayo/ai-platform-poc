...
| コアライブラリ + CLI + API を提供 | **実装済み** | `ai_platform_samplelib/application/autonomous/api/api_server.py` / `.../mcp/mcp_server.py` / `.../cli/main.py` | API は FastAPI、MCP は FastMCP（stdio/sse/http）を提供。 |
...
5. MCP 認証情報の「受け取り→workspace内設定反映」の経路が薄い（部分解消）
   ...
   * 現状（部分解消）:
      * executor は `Authorization` / `trace_id` を受領し、下流（subprocess/コンテナ）へ **タスク単位の環境変数**として注入できるようにした。
      * 一方で、`create_opencode_json.py` は LLM 接続設定（provider/model/baseURL）中心で、`config/common/opencode.jsonc` の MCP 定義は固定値のまま。
   * 未対応: opencode 等が参照する workspace 内設定（例: `opencode.json(c)`）へ **動的に反映**する仕組み（`{env:...}` 参照や `OPENCODE_CONFIG_CONTENT` のタスク単位注入）は別途実装が必要。
...
* 自律型エージェントが呼び出すMCPツールには認証情報(Bearerトークンなど)を渡す必要がある場合がある。
   ...
   * workspace 内の設定ファイル（例: opencode の設定）への反映は別途実装する。
...
| MCP 認証（Bearer等）の受け取り→workspace 内設定へ反映 | **部分的** | executor: `ai_platform_samplelib/application/common/request_headers.py` / `.../application/autonomous/core/*_coding_agent_runner.py` | **現状**: Authorization 等は受領して下流へ環境変数で注入できる。**未対応**: opencode等の設定ファイル（workspace内）へ動的反映する仕組みは別途。 |
