...
* サンプル実装の方針
   ...
   * Dockerコンテナの操作をPythonから行うサンプル実装のテストクライアントを[test/application/autonomous-agent-executor](../test/application/autonomous-agent-executor)に配置する。
...
1. （解消）手順書にあるCLIパスの誤り
   * 方針/期待: CLI（テストクライアント）から自律型エージェントの実行ができること。
   * 現状: テストクライアントは `test/application/autonomous-agent-executor`（例: `test_client.sh`）にあり、PoC手順側の誤記（`app/docker/autonomous-agent-executor/cli`）は修正済み。
   * 影響: 手順書が最新であれば詰まらない（オンボーディング/再現性の改善）。
