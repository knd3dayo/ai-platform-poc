...
* SV型エージェントは実行全体で一意の `trace_id` を採番し、自律型エージェント実行結果の `TaskStatus.trace_id` として伝播する。
   * 補足: SVはサブタスクごとに executor を複数回起動しうるため、executor の `task_id` は workspace/compose project 名の衝突回避の観点で別途一意化（UUID等）する。
...
3. トレースID（`trace_id`）の伝播（SV→自律）は解消（`TaskStatus.trace_id` に格納）
   * 方針/期待: SV実行全体で一意な `trace_id` を採番し、自律型エージェントの `TaskStatus.trace_id` として伝播する（相関ID）。
   * 現状:
      * SV実行全体で `trace_id` を採番し、SV→自律の呼び出し引数として渡し、自律側の `TaskStatus.trace_id` に保存している。
      * 一方で executor の `task_id` は、サブタスクごとに別採番（UUID等）している（workspace/compose project 名の衝突回避のため）。
      * pause/resume のためのHITLセッションJSONにも `trace_id` を保存し、再開後も同一 `trace_id` を引き継ぐ。
   * 影響/評価:
      * `trace_id` による横断的な相関（観測/運用）は可能になった。
      * `task_id` と `trace_id` は役割が異なるため、strict に `trace_id = task_id` は要件としない（必要になった場合は別途設計する）。
