...
**【能力の獲得】** AIが高度な自然言語処理能力（推論エンジン）を獲得した。
**【脅威とガバナンスの進化】**
...
これを防ぐため、API Gatewayの奥に、入出力を意味的に監視する専用レイヤーを配置するアーキテクチャが生まれた。Metaの『[Llama Guard](https://arxiv.org/abs/2312.06674)』やNVIDIAの『[NeMo Guardrails](https://docs.nvidia.com/nemo/guardrails/)』に代表されるように、**小型のLLMをファイアウォールとして配置し「意味論的ガードレール」として機能させるアプローチ**がエンタープライズの実装標準となった。

### 第2期：ハルシネーションの実害と「説明責任」の確立（2023年末〜2024年）
**【能力の獲得】** AIが社内文書やWeb検索を組み合わせて、もっともらしい長文回答（RAG等）を生成するようになった。
**【脅威とガバナンスの進化】**

AIの「ハルシネーション（もっともらしい嘘）」が実務で致命傷になるリスクが露呈した。スタンフォード大学等の論文（[*Evaluating Verifiability in Generative Search Engines*](https://arxiv.org/abs/2304.09848), 2023）により、AIが生成した文章のうち「引用（サイテーション）によって完全に裏付けられているものはわずか51.5%に過ぎない」ことが実証された。
...
**【能力の獲得】** AIが自律的に外部システムやファイル操作を行う「エージェント（Tool利用）」へと進化した。
**【脅威とガバナンスの進化】**
...
**【能力の獲得】** AIが目標に向けて「思考→行動→観察」のループを自律的に回し続けるようになった。
**【脅威とガバナンスの進化】**
...
**【能力の獲得】** AIが単体で推論・実行するだけでなく、複数のエージェントが役割分担し、相互に依頼・委譲・合議しながらタスクを遂行するようになった。
**【脅威とガバナンスの進化】**
...
エージェントの自律化に伴うブラックボックス化を防ぐため、入口で確定した `trace_id` を全レイヤーに伝播させ、AIガバナンス層で一元的に観測・監査する基盤を構築している。これにより、事後の原因究明と監査可能性をシステムとして保証する。
* **準拠する要件・参考文献**: 経済産業省 『AI事業者ガイドライン（第1.2版）』, NIST 『AI RMF 1.0』 (Measure機能群), 『Langfuse』
...
* **準拠する要件・参考文献**: 『Not what you've signed up for (間接インジェクションの脅威)』, 『SWE-agent (Agent-Computer Interface)』
...
* **準拠する要件・参考文献**: 学術論文 『Measuring Progress on Scalable Oversight』, 『RAGAS』, 『Judging LLM-as-a-Judge』, 経済産業省 『AI事業者ガイドライン（第1.2版）』
...
* **準拠する要件・参考文献**: 『Taming Various Privilege Escalation in LLM-Based Agent Systems』, 『Agent Smith』, 『CORBA』, 『TrinityGuard』
...
| **検証不可能性** | 出典メタデータ付与、同期検証でのサイテーション有無確認、trace_id ベース監査、事後評価 | AIガバナンス層 + Application層 | 予防・検知・追跡 | 単なる引用要求ではなく、根拠付けをシステムで強制し、欠落時は次ステップへ進ませない。 |
...
| **マルチエージェント間の権限昇格・汚染伝播・再帰的ブロッキング** | 委譲時ポリシー検証、State Scrubbing、agent-to-agent Guardrails、セッション予算監視、trace 連結監査 | AIガバナンス層 + Application層 + 観測基盤 | 予防・検知・停止・追跡 | 単一エージェントの入口検査だけでは不十分であり、エージェント間通信そのものをゼロトラストで扱う必要がある。 |
| **スケーラブルな監督限界** | trace_id 中心の Observability、共通評価基盤、管理者UI | AIガバナンス層 + BFF + 観測基盤 | 検知・追跡 | 人間が全ログを読む前提を捨て、自動評価と検索可能な監査に置き換える。 |
...
* **他層との協調で成立する統制**: trace_id 伝搬、非同期 HITL、ユーザー権限継承、Tool の実行制約、agent-to-agent handoff の検証。
...
### 4.4 trace_id を軸にした追跡可能性を全層に通す

trace_id は主に予防ではなく、横断追跡、監査、原因究明のための基盤である。BFF で確定した trace_id を Application層、Tool層、評価基盤、通知経路まで一貫して伝播させることで、人間が個別ログを寄せ集めなくても、一連のリクエストを検索・再構成できる状態を作る。
...
この原則は、4.3 の権限最小化を agent-to-agent handoff にまで拡張し、4.4 の trace_id を複数エージェント間の因果追跡へ広げ、4.5 の Risk-Adaptive 監督を単一ループだけでなく複数エージェント間の相互作用へ適用するものである。すなわち、内部委譲を信頼せず、必要最小限の権限、浄化済み State、監査可能な handoff だけを通すことが、East-West 統制の前提になる。
...
### 5.6 trace_id 中心の Observability

North境界で確定した trace_id を起点に、入力、根拠データ取得、出力、Tool 実行、評価結果、停止判断を一気通貫で記録する。

* **主な効能**: 事後追跡、インシデント解析、監査証跡の一元化。
* **設計上の注意**: trace_id は防御の主役ではなく、統制が効いたかどうかを後から検証し、運用で改善するための基盤である。マルチエージェント構成では、親子の handoff 関係、委譲元・委譲先エージェント ID、共有 State の版、予算消費履歴も追跡対象に含める必要がある。
...
レスポンス後またはバックグラウンドで、観測基盤へ蓄積された trace を共通評価基盤が読み込み、忠実性、サイテーション妥当性、安全性、ポリシー適合性を採点する。低スコア時には HITL、明白な危険兆候時には Kill Switch を発火させる。
...
* **事後追跡が主目的の機構**: trace_id、観測基盤、監査証跡。
* **封じ込めと停止が主目的の機構**: サンドボックス、Egress 制御、Kill Switch。

この区別を明確にすることで、trace_id を過大評価したり、Guardrails に封じ込めまで期待したり、事後評価だけでリアルタイム安全性を担保できると誤認したりする設計誤解を避けられる。
...
| **横断的な監査・追跡** | **AIガバナンス層** | trace 単位で全レイヤーを串刺しにする必要があるため。 |
...
| **ハイブリッド構成** | 型切替点での trace_id 維持、責務分界の継続、agent-to-agent handoff の検証 | 監査単位の分断防止と権限・状態の不正伝播防止 |
...
| **検証不可能性** | **対応済み** | 出典メタデータ、同期検証でのサイテーション確認、trace_id、共通評価により、説明責任をシステム証跡として残せる。 | サイテーションの意味妥当性は 事後評価 の閾値設計と監査運用に依存する。 |
...
| **マルチエージェント間の権限昇格・汚染伝播・再帰的ブロッキング** | **多層前提で対応** | 委譲時ポリシー検証、State Scrubbing、セッション予算監視、trace 連結監査を追加することで、単体エージェントでは見えない内部リスクを扱える。 | Application層の handoff 実装が粗い、または共有 State の浄化が弱い場合、内部通信が抜け道になる。 |
| **スケーラブルな監督限界** | **対応済み** | trace_id 中心の Observability と共通評価基盤により、人間の全件目視を前提にしない構成になっている。 | 管理者 UI やアラート運用が弱いと監督能力が形骸化する。 |
...
* **概要**: LLMアプリケーションのトレース、評価を行うObservabilityプラットフォーム。 `trace_id` を用いて一連の動作を可視化・監査するための実装リファレンス。
...
* **アーキテクチャとの関係**: システム全体で1つのリクエストを `trace_id` で串刺しにし、Application層やTool層の全アクションを相関づけて記録する**「Observability（観測基盤）」**が、単なる運用上の便利ツールではなく、自律型エージェントのデバッグと評価（LLM-as-a-Judge）を成立させるための**学術的・構造的な必須要件**であることを証明する。
