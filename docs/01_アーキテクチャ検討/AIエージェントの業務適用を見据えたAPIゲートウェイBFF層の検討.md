  ...
  * 画面再ロード時のエージェント思考プロセスの消失
  ...
  > 注：生成AIの回答チェックやガードレール等の **意味論的な統制** は、BFFに閉じず、専用のAIガバナンスコンポーネント（例：LiteLLM Proxy / MCPゲートウェイ）へ寄せるのが推奨です。
  ...
  * **分散トレーシング**: 入口→BFF→Application→ガバナンス→Tool→（Event Bus）を串刺しにする。
