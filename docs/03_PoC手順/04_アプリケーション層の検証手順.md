...
# Textual は optional extra（tui）です
pip install -e "$AI_PLATFORM_POC_ROOT/app/ai-platform-samplelib[tui]"

python -m ai_platform_samplelib.application.super_visor.tui.app
```
...
```bash
. ${AI_PLATFORM_LIB}/.venv/bin/activate
python -m ai_platform_samplelib.application.super_visor.cli.main run \
