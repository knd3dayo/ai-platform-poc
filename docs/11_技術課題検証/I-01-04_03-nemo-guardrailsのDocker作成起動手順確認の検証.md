...
- `docker stats --no-stream` 時点で NeMo Guardrails コンテナは約 `171 MiB` を使用していた。
- `/proc/1/status` の確認では `VmRSS` は約 `173 MiB`、`Threads` は 5 だった。
- LiteLLM よりは軽量だが、Colang のイベント処理とガードレール判定を常駐で持つため、単純な HTTP 中継よりはメモリを使用する。
