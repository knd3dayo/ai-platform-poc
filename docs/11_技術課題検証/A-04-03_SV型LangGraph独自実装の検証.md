...
- pause 応答で `trace_id` を追跡でき、同一 `trace_id` を使う resume 契約へ接続できること。
...
| 制御成立性 | absolute path を含む directory 確認要求で `general_tool_agent -> analyze_files -> paused` が成立し、探索要求では `deep_agent` へ切り替わる。deep_agent 側の完了品質は別途 path 解釈品質の影響を受ける。 |
| 運用成立性 | `trace_id`、route、tool 名、approval 理由、`final_status` を監査ログから再構成でき、resume は same `trace_id` を使う運用で説明できる。 |
...
| generic directory 停止 | 確認済み | trace_id `1ff4a515816a4b62b95bef3178c0f07c`。`paused` は再現したが、reason code は `route.general_tool_sufficient` だった。 |
| deep_agent route 境界 | 一部確認済み | trace_id `e4e63e84e13a4d0eb8f07c29d48d1ad2`。`deep_agent` への route は確認したが、最終状態は `completed` ではなく `paused` だった。 |
| pause 後の再開接続点 | 確認済み | absolute path / generic の両シナリオで pause 応答と trace_id を確認した。resume 契約の正本は A-02-02 を参照する。 |

## 残課題

- generic な directory 問い合わせの reason code を `route.explicit_directory_path_request` に寄せるか、現状の汎用ラベルを仕様として受け入れるかは別論点として整理が必要である。
- DeepAgents 側の directory path 解釈品質は A-04-03 の受け入れ条件には含めず、A-04-04 / A-04-06 側で継続確認する必要がある。
- `work ディレクトリ全体を起点に深く調査してください` の live 実行では、`deep_agent` が `/home/user/source/repos/ai-platform-poc/work` を解決しに行き、`Path not found` で `paused` へ倒れた。境界確認は成立したが、完了品質は別途改善が必要である。
- same `trace_id` を使う再開の end-to-end 検証は、A-02-02 と組み合わせて API / ライブラリ経路を正本として継続確認する必要がある。
