...
また、Supervisor が選択対象とする実装基盤そのものの成立性については、A-04 系の文書を前提情報として参照する。
...
```bash
export AI_CHAT_UTIL_RUNNER="${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/run-ai-chat-util.sh"
export AI_CHAT_UTIL_CONFIG="${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.structured-routing.poc.yml"
export AUDIT_LOG_PATH="${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/work/structured-routing-audit.jsonl"
export TARGET_WORKSPACE="${HOME}/source/repos/ai-platform-poc"
...
- 実行ランナー:
  - `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/run-ai-chat-util.sh`
- 使用設定ファイル:
  - `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.structured-routing.poc.yml`
- MCP 設定:
  - `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/mcp_servers.local.json`
- 監査ログ:
  - `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/work/structured-routing-audit.jsonl`
- 主な有効化設定:
  - `routing_mode: structured`
  - `sufficiency_check_enabled: true`
  - `audit_log_enabled: true`
  - `audit_log_path: ${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/work/structured-routing-audit.jsonl`
- 対象ワークスペース:
  - `${HOME}/source/repos/ai-platform-poc`
...
## 追記: fresh run 実測結果（2026-04-06）

2026-04-06 時点で、structured routing の fresh run を ai-chat-util CLI から再実行し、A-01-02 の主要論点を再確認した。

### 実施条件

- 実行コマンド: `uv run ai-chat-util --config /home/user/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.structured-routing.poc.yml agent_chat -p ...`
- 使用設定ファイル:
...
| シナリオ | 結果 | 補足 |
| --- | --- | --- |
| 通常ツール選択 | OK | `現在読み込まれている設定ファイルの場所を教えてください。` に対し、`route_decided=general_tool_agent`、`tool_selected=get_loaded_config_info`、`sufficiency.answer_supported_by_evidence` を確認した。 |
| coding-agent 選択 | OK | `docs 配下を調査して、共通している見出しを3点に整理してください。必要なら coding-agent を使ってください。` に対し、出力として見出し 3 点を返し、監査ログでも `route_decided=coding_agent` を確認した。 |
| 情報不足時の確認要求 | OK | `この設定で本番投入してよいですか。判断に足りない点があれば明示してください。` に対し、CLI 出力は追加確認事項を列挙し、監査ログでは `route_decided=reject`、`requires_clarification=true`、`hitl_requested`、`final_status=paused` を確認した。 |
...
- 情報不足時の監査ログ抜粋:
  ...
  route_name=reject
  reason_code=route.reject_unsupported_request
...
- A-01-02 の主論点である「通常ツールへの委譲」「非同期 job 系への委譲」「情報不足時の確認要求」は、2026-04-06 の fresh run でも成立した。
- HITL は「確認点を返すだけ」ではなく、監査ログ上で `hitl_requested` と `paused` を伴う外部契約として観測できた。
- 追加で DeepAgent 優先シナリオを再試験したところ、ディレクトリ単位の探索依頼では `route_decided=deep_agent` までは到達するが、`指定されたディレクトリ ... には Markdown ファイルが見つかりませんでした` となり、`missing_user_context` 扱いで `paused` に落ちた。
- 一方、比較対象ファイルを明示した再試験では `route_decided=deep_agent`、`reason_code=route.multi_step_investigation_needed`、`final_answer_validated=sufficiency.answer_supported_by_evidence` まで到達し、見出し 3 点の整理を完了できた。

### 追記: DeepAgent 優先シナリオ再試験（2026-04-06）

DeepAgent 優先設定の安定性を確認するため、同日中に 2 パターンの追加シナリオを実施した。

| シナリオ | 結果 | 補足 |
| --- | --- | --- |
| ディレクトリ単位の探索依頼 | 部分成立 | `route_decided=deep_agent` までは到達したが、`/home/user/source/repos/ai-platform-poc/docs/11_技術課題検証` を対象にした探索は `missing_user_context` となり、`hitl_requested` / `paused` に落ちた。 |
| ファイル比較タスク | OK | 対象 2 ファイルを明示した比較タスクでは、DeepAgent ルートで完了し、見出し 3 点を返した。 |

#### ディレクトリ単位探索の出力抜粋

```text
指定されたディレクトリ "/home/user/source/repos/ai-platform-poc/docs/11_技術課題検証" 内には、Markdown ファイルが見つかりませんでした。
設定ファイルの場所: /home/user/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.structured-routing.poc.yml
```

監査ログ上は `trace_id=2740aeaa42954982a729036b24a920d8` で以下を確認した。

```text
route_name=deep_agent
reason_code=route.multi_step_investigation_needed
sufficiency.answer_supported_by_evidence
```
...
監査ログ上は `trace_id=5f6d8e186fd243019875891437d94795` で以下を確認した。

```text
route_name=deep_agent
reason_code=route.multi_step_investigation_needed
final_answer_validated=sufficiency.answer_supported_by_evidence
```

#### DeepAgent 再試験の所見

- DeepAgent 優先設定そのものは有効であり、比較タスクのように対象ファイルを明示した multi-step inquiry では安定して `deep_agent` へ収束した。
- 一方で、ディレクトリ探索を自然言語で委ねた場合は、存在する path でも探索対象の解釈が不安定で、`missing_user_context` 扱いへ落ちるケースが残った。
- したがって A-01-02 の観点では、「DeepAgent ルートの選択と完了」は確認済みとしつつ、「ディレクトリ探索を伴う汎用調査 prompt の安定性」は継続改善対象と整理するのが妥当である。

### 追記: DeepAgent ディレクトリ探索不安定性の原因調査（2026-04-06）

ai-chat-util 実装を追跡した結果、DeepAgent のディレクトリ探索不安定性は主として prompt 品質ではなく、ファイルパス解決実装の制約に起因することを確認した。

#### 原因の要約

1. `analyze_files` 系ツールの入力解決は `resolve_existing_file_path()` に集約されているが、最終判定が `Path(c).is_file()` 固定であるため、ディレクトリ指定は存在していても解決成功扱いにならない。
2. 候補パスの試行順は `input as-is`、`cwd 基準の相対パス`、その後に `working_directory` となっており、structured-routing PoC のように `working_directory=/home/user/source/repos/ai-platform-poc` を持つケースでも、期待される候補が後順位になる。
3. DeepAgent の system prompt には「ローカルファイルパスを指定された場合は確認する」指示はあるが、`working_directory` を基準に相対パスを解決する指示や、ディレクトリ入力をどのツールへどう渡すべきかの明示がない。

#### 根拠と確認箇所

- `src/file_util/util/file_path_resolver.py`
  - `resolve_existing_file_path()` は `working_directory` を候補に追加しているが、最終的な成功条件が `Path(c).exists() and Path(c).is_file()` である。
- `src/ai_chat_util/analysis/analysis_service.py`
  - `resolve_existing_file_paths()` から上記 resolver を `working_directory=runtime_config.mcp.working_directory` 付きで呼び出している。
- `src/ai_chat_util/core/tool_app.py`
  - `analyze_files()` は `_resolve_existing_file_paths()` を経由するため、DeepAgent でも最終的には同じ file-only 制約に入る。
- `src/ai_chat_util/base/agent/deep_agent_support.py`
  - DeepAgent 用 system prompt には、ローカルファイル確認の指示はあるが、path normalization と directory handling に関する強い指示はない。
- `src/ai_chat_util/base/agent/agent_client_util.py`
  - `create_deep_agent_workflow()` では `build_deep_agent_system_prompt()` をそのまま渡しており、`working_directory` をプロンプト側へ注入していない。

#### 解釈

- 今回の `deep_agent` 失敗は「DeepAgent が選ばれなかった」のではなく、「DeepAgent は選ばれたが、ディレクトリ指定を file-oriented な解析ツールへ渡した時点で path resolver に吸収され、存在する path を成功扱いできなかった」と解釈するのが正確である。
- したがって、A-01-02 の残課題は routing 精度そのものより、`deep_agent` と file analysis tool の間にある path contract の改善と考えるべきである。

#### ai-chat-util へ返却したい事項（追加）

4. `resolve_existing_file_path()` が `is_file()` 固定のため、存在するディレクトリ入力でも解決失敗になる。DeepAgent から directory-oriented inquiry を投げる場合の契約と不整合がある。
5. `working_directory` は resolver へ渡されているが、候補順が `cwd` 優先で後順位になる。PoC のような別リポジトリ参照では、`working_directory` 基準候補を先に試す方が妥当である。
6. DeepAgent system prompt に `working_directory` 基準の path 解決方針を明示しない限り、自然言語でのディレクトリ探索依頼は再発しやすい。
...
- API サーバー:
  - `uv run -m ai_chat_util.api.api_server --config ${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.structured-routing.hitl.poc.yml`
- 使用設定ファイル:
  - `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.structured-routing.hitl.poc.yml`
- 追加設定:
  - `routing_mode: structured`
  - `sufficiency_check_enabled: true`
  - `audit_log_enabled: true`
...
- 監査ログ:
  - `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/work/structured-routing-hitl-audit.jsonl`
- 対象 trace_id:
...
- MCP 設定: `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/mcp_servers.local.json`
- 設定ファイル: `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/ai-chat-util-config.poc.yml`
- 対象ワークスペース: `${HOME}/source/repos/ai-platform-poc`
- 実行ランナー: `${HOME}/source/repos/ai-platform-poc/infra/31-ai-chat-util-mcp/run-ai-chat-util.sh`
