...
## 前提
* [02_AIエージェントの業務適用を見据えた生成AIアプリケーション層の検討.md](../01_アーキテクチャ検討/02_AIエージェントの業務適用を見据えた生成AIアプリケーション層の検討.md)をベースとする。

## 概要
### 全体概要
#### 最終的なイメージ
* クライアント -> BFF -> アプリケーション(WF型エージェント/SV型エージェント) ->  自律型エージェントという一連の流れを実現する。
* ユーザーは、クライアントアプリに質問、各種データが格納されたファイルサーバー上のワークスペースのパスを入力する。
* BFFはIdPと連携して、認証用のBearerトークンを発行する。また、Application層/Tool層の処理をトレースするためのトレースIDを入力とする。
* 自律型エージェントは、Claude Code、Cline CLI、OpenCodeのようなコーディングエージェントをとし、サンドボックス内で実行する。  
  サンドボックスとしてDockerコンテナを使用する。
* 自律型エージェント/サンドボックス起動用のMCPサーバーを用意する。これを自律型エージェント用MCPサーバーとする。
* SV型エージェントはLangGraphを用いて実装する。これにFastAPIによるAPI機能を付与して、BFFからの呼び出しを可能とする。

#### 初期段階のイメージ
* クライアント -> BFF -> アプリケーション(WF型エージェント/SV型エージェント) ->  自律型エージェントという一連の流れを実現する。
  ⇒ クライアントアプリはBFF経由ではなくバックエンドのアプリケーションに直接アクセスする。
* ユーザーは、クライアントアプリに質問、各種データが格納されたファイルサーバー上のワークスペースのパスを入力する。  
  ⇒ ワークスペースのパスはファイルサーバーではなく、ローカルファイルシステム上のパスとする。
* BFFはIdPと連携して、認証用のBearerトークンを発行する。また、Application層/Tool層の処理をトレースするためのトレースIDを入力とする。  

* 自律型エージェントは、Claude Code、Cline CLI、OpenCodeのようなコーディングエージェントをとし、サンドボックス内で実行する。  
  サンドボックスとしてDockerコンテナを使用する。  
  ⇒ Dockerコンテナ起動は複雑&リソース消費が激しいのでまずはサブプロセスとしてコーディングエージェントを起動する。
* 自律型エージェント/サンドボックス起動用のMCPサーバーを用意する。これを自律型エージェント用MCPサーバーとする。
  ⇒ Dockerコンテナ起動は複雑&リソース消費が激しいのでまずはサブプロセスとしてコーディングエージェントを起動する
* SV型エージェントはLangGraphを用いて実装する。これにFastAPIによるAPI機能を付与して、BFFからの呼び出しを可能とする。
  ⇒ APIの実装を行う。BFFから渡されたBearerトークンとトレースIDを受け付ける処理を実装する。
